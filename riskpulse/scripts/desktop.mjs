...
const server=http.createServer(async(req,res)=>{
 ...
 try{
  ...
  if(['/app','/dashboard.html'].includes(path)&&!user.localOwner&&(!user.plan||user.plan==='pending')){res.writeHead(303,{Location:'/plans','Cache-Control':'no-store'});return res.end();}
